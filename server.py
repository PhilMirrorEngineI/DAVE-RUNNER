...
def _jfail(msg: str, http: int = 400, **extra):
    p = {"ok": False, "error": msg}; p.update(extra)
...
def _get_json() -> Tuple[Optional[dict], Optional[Tuple[Any, int]]]:
    try:
        ...
        if not isinstance(d, dict): return None, _jfail("JSON body must be an object", 400)
        return d, None
    except Exception: return None, _jfail("Invalid or missing JSON body", 400)
...
def reflect():
    ...
    status="OK"
    if abs(drift_in)>=stop:status="STOP"
    elif abs(drift_in)>=warn:status="WARN"
    return _jok({
        "lawful":True,"status":status,"user_id":uid,"thread_id":tid,
        "slide_id":sid,"glyph_echo":glyph,"drift_in":drift_in,
        "drift_clamped":drift,"thresholds":{"clamp":clamp,"warn":warn,"stop":stop},
        "reflection":_clip(content,6000),"ts":int(time.time())
    })

# ---------- Memory passthrough ----------
@app.route("/memory/save", methods=["POST"])
def memory_save():
    if not _mem_enabled(): return _jfail("Memory API not configured",503)
    d,err=_get_json()
    if err:return err
    try:
        ...
        return jsonify({"ok":r.ok,"upstream_status":r.status_code,"data":_safe_json(r)}),(200 if r.ok else 502)
