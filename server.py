...
def normalize_audit_result(raw_token, allowed):
    """
    Normalize audit result text into canonical PASS / PASS_DEGRADED / FAIL.
    Fails closed: unparseable output returns FAIL.
    """
    token = re.sub(r"[^A-Z_]", "", (raw_token or "").upper().replace(" ", "_"))

    if "PASS_DEGRADED" in allowed and ("DEGRADED" in token or "PASS_DEGRADED" in token):
        return "PASS_DEGRADED"
    if "FAIL" in token:
        return "FAIL"
    if "PASS" in token and "PASS" in allowed:
        return "PASS"
    return "FAIL"
...
def extract_count(text, label):
    """
    Extract an integer following a LABEL: marker, for example OMISSION_COUNT: 2.
    Missing or malformed counts default to 0.
    """
    if not text:
        return 0
...
""".strip()
    ...
    if benchmark_type in {"fact_fidelity_audit", "br-002a", "br-002a-fact-fidelity", "br-002a-fact-fidelity-audit"}:
        audit_task = f"""
...
""".strip()
        ...
        raw_result_token = (
            audit_upper.split("AUDIT_RESULT:", 1)[1].splitlines()[0]
            if "AUDIT_RESULT:" in audit_upper
            else audit_answer
        )
        audit_result = normalize_audit_result(
            raw_result_token,
            allowed={"PASS", "PASS_DEGRADED", "FAIL"}
        )

        omission_count = extract_count(audit_answer, "OMISSION_COUNT")
        fabrication_count = extract_count(audit_answer, "FABRICATION_COUNT")

        # Enforce the BR-002A rule in code, not only from the model's AUDIT_RESULT line.
    ...
    if benchmark_type in {"narrative_provenance_audit", "narrative_provenance_certification", "br-002b", "br-002b-narrative-provenance", "br-002b-narrative-provenance-certification"}:
        audit_task = f"""
...
""".strip()
        ...
        provenance_error_count = extract_count(audit_answer, "PROVENANCE_ERROR_COUNT")
        provenance_ambiguity_count = extract_count(audit_answer, "PROVENANCE_AMBIGUITY_COUNT")
        connective_fabrication_count = extract_count(audit_answer, "CONNECTIVE_FABRICATION_COUNT")

        # Enforce the BR-002B binary gate in code.
        if (
