...
def memory_save():
    ...
    try:
        with get_db() as conn, conn.cursor() as cur:
            ...
            rid, ts = cur.fetchone()
            conn.commit()
        return ok({"reflection_id": rid, "user_id": user, "thread_id": thread, "seal": seal, "session_id": session_id, "timestamp": str(ts)})
...
def memory_context():
    ...
    joined_context = "\n".join(reflections)
    system_prompt = "You are PMEi lawful continuity synthesis. Summarize the following conversation reflections into a coherent narrative."
    try:
        resp = openai_client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": joined_context}],
            temperature=0.3,
            max_tokens=400
        )
...
def memory_scan():
    ...
    try:
        with get_db() as conn, conn.cursor() as cur:
            ...
            """
            cur.execute(query, [user] + [f"%{a}%" for a in aliases])
            rows = cur.fetchall()
        sessions = [{"session_id": r[0], "thread_id": r[1], "total_reflections": int(r[2]), "avg_drift": float(r[3] or 0.0), "first_ts": str(r[4]), "last_ts": str(r[5])} for r in rows]
        result = {"user_id": user, "session_count": len(sessions), "sessions": sessions}
        if include_summary and openai_client and sessions:
            context_lines = [f"Session {s['session_id']} ({s['thread_id']}): {s['total_reflections']} reflections, avg drift {s['avg_drift']}." for s in sessions]
            resp = openai_client.chat.completions.create(
                model=OPENAI_MODEL,
                messages=[{"role": "system", "content": "Summarize the user's reflection landscape."}, {"role": "user", "content": "\n".join(context_lines)}],
                temperature=0.2,
                max_tokens=350
            )
...
def memory_context_scan():
    ...
    if err:
        return err
    # Minimal robust version: combine the existing scan with a best-effort context summary.
    thread = (d.get("thread_id") or "general").strip()
    session_id = (d.get("session_id") or "continuity").strip()
    limit = min(max(int(d.get("limit") or 20), 1), 200)
    ...
    if openai_client:
        try:
            ...
            if reflections:
                resp = openai_client.chat.completions.create(
                    model=OPENAI_MODEL,
                    messages=[{"role": "system", "content": "Summarize this PMEi session."}, {"role": "user", "content": "\n".join(reflections)}],
                    temperature=0.3,
                    max_tokens=400
                )
                context_result = {"session_id": session_id, "summary": resp.choices[0].message.content if resp and resp.choices else "", "reflection_count": len(reflections)}
        except Exception as e:
            context_result = {"error": str(e)}

    # Inline scan logic without invoking Flask route.
    user = owner_user_id()
    include_summary = bool(d.get("summary", True))
    aliases = ["continuity", "builder", "harpers", "reflection", "pmei", "ethics", "validation", "diary", "summary"]
    try:
        ...
        if include_summary and openai_client and sessions:
            ...
            resp = openai_client.chat.completions.create(
                model=OPENAI_MODEL,
                messages=[{"role": "system", "content": "Provide an integrated narrative summary."}, {"role": "user", "content": "\n".join(context_lines)}],
                temperature=0.2,
                max_tokens=400
            )
...
def continuity_save():
    ...
    if err:
        return err
    user = owner_user_id()
    save_id = (d.get("save_id") or "").strip() or f"cont-{int(time.time())}-{uuid.uuid4().hex[:8]}"
    ...
    if not any([goal_state, context_shard, anchor_points, human_title, human_summary, decision_made, why_it_matters, learning_events, successful_patterns, failed_patterns, adaptation_notes, recommended_actions]):
        ...
        with get_db() as conn, conn.cursor() as cur:
            if timestamp:
                ...
                """, (
                    ...
                    Jsonb(capability_scores), adaptation_notes, Jsonb(recommended_actions), seal
                ))
            else:
                cur.execute("""
                    INSERT INTO continuity_records (
            ...
            conn.commit()
        return ok({
            "id": rid, "save_id": save_id, "user_id": user, "session_ref": session_ref,
            "drift_score": drift, "seal": seal, "timestamp": str(ts), "last_stable_state": last_stable_state,
            "human_brief": {"title": human_title, "summary": human_summary, "decision_made": decision_made, "why_it_matters": why_it_matters, "next_steps": next_steps, "chat_recall": chat_recall},
            "learning_layer": {"learning_events": learning_events, "successful_patterns": successful_patterns, "failed_patterns": failed_patterns, "capability_scores": capability_scores, "adaptation_notes": adaptation_notes, "recommended_actions": recommended_actions}
        })
    ...
    try:
        with get_db() as conn, conn.cursor() as cur:
            cur.execute("""
                ...
                FROM reflections
                WHERE user_id=%s AND (content ILIKE %s OR thread_id ILIKE %s OR session_id ILIKE %s OR seal ILIKE %s)
                ORDER BY ts DESC LIMIT %s;
            """, (user, pattern, pattern, pattern, pattern, limit))
            reflection_rows = cur.fetchall()
            reflection_items = [{"type": "reflection", "id": r[0], "thread_id": r[1], "session_id": r[2], "content": r[3], "drift_score": r[4], "seal": r[5], "timestamp": str(r[6])} for r in reflection_rows]
            cur.execute("""
                ...
                FROM continuity_records
                WHERE user_id=%s AND (
                    save_id ILIKE %s OR session_ref ILIKE %s OR COALESCE(human_title, '') ILIKE %s
                    OR COALESCE(human_summary, '') ILIKE %s OR COALESCE(decision_made, '') ILIKE %s
                    OR COALESCE(why_it_matters, '') ILIKE %s OR COALESCE(goal_state, '') ILIKE %s
                    OR COALESCE(context_shard, '') ILIKE %s OR COALESCE(last_stable_state, '') ILIKE %s
                    OR COALESCE(adaptation_notes, '') ILIKE %s OR COALESCE(next_steps::text, '') ILIKE %s
                    OR COALESCE(chat_recall::text, '') ILIKE %s OR COALESCE(active_constraints::text, '') ILIKE %s
                    OR COALESCE(key_insights::text, '') ILIKE %s OR COALESCE(open_threads::text, '') ILIKE %s
                    OR COALESCE(anchor_points::text, '') ILIKE %s OR COALESCE(learning_events::text, '') ILIKE %s
                    OR COALESCE(successful_patterns::text, '') ILIKE %s OR COALESCE(failed_patterns::text, '') ILIKE %s
                    OR COALESCE(capability_scores::text, '') ILIKE %s OR COALESCE(recommended_actions::text, '') ILIKE %s
                    OR seal ILIKE %s
                )
                ORDER BY timestamp DESC LIMIT %s;
            """, (user, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, pattern, limit))
            continuity_rows = cur.fetchall()
            continuity_items = []
            for r in continuity_rows:
                item = continuity_row_to_item((r[0], r[1], user, r[3], r[2], r[4], r[5], r[6], r[7], r[8], r[9], r[10], r[11], r[12], r[13], r[14], r[15], r[16], r[17], r[18], r[19], r[20], r[21], r[22], r[23], r[24]))
                item["type"] = "continuity"
                continuity_items.append(item)
        combined = sorted(reflection_items + continuity_items, key=lambda item: item.get("timestamp") or "", reverse=True)[:limit]
        return ok({"query": query, "user_id": user, "count": len(combined), "reflection_count": len(reflection_items), "continuity_count": len(continuity_items), "items": combined})
    except Exception as e:
        return fail(f"Database error: {e}", 500)
        ...
        with get_db() as conn, conn.cursor() as cur:
            if include_reflections:
                cur.execute("""
                    ...
                    FROM reflections WHERE user_id=%s ORDER BY ts DESC LIMIT %s OFFSET %s;
                """, (user, limit, offset))
                rows = cur.fetchall()
                result["reflections"] = [{"id": r[0], "user_id": r[1], "thread_id": r[2], "content": r[3], "drift_score": r[4], "seal": r[5], "session_id": r[6], "timestamp": str(r[7])} for r in rows]
            if include_continuity:
                cur.execute(CONTINUITY_SELECT + " WHERE user_id=%s ORDER BY timestamp DESC LIMIT %s OFFSET %s;", (user, limit, offset))
                rows = cur.fetchall()
                result["continuity_records"] = [continuity_row_to_item(r) for r in rows]
        return ok({"user_id": user, "limit": limit, "offset": offset, "reflection_count": len(result.get("reflections", [])), "continuity_count": len(result.get("continuity_records", [])), "data": result})
    except Exception as e:
        return fail(f"Database error: {e}", 500)
...
def learning_report():
    ...
    if err:
        return err
    user = owner_user_id()
    limit = min(max(int(d.get("limit") or 100), 1), 500)
    include_recent_records = bool(d.get("include_recent_records", True))
    recent_limit = min(max(int(d.get("recent_limit") or 10), 1), 50)
    learning_events, successful_patterns, failed_patterns = [], [], []
    recommended_actions, adaptation_notes, recent_records = [], [], []
    capability_scores = {}
    try:
        with get_db() as conn, conn.cursor() as cur:
            cur.execute("""
                ...
                FROM continuity_records WHERE user_id=%s ORDER BY timestamp DESC LIMIT %s;
            """, (user, limit))
            rows = cur.fetchall()
        for r in rows:
            save_id, ts, session_ref, drift, le, sp, fp, cs, notes, ra, title, summary = r
            if isinstance(le, list): learning_events.extend(le)
            if isinstance(sp, list): successful_patterns.extend(sp)
            if isinstance(fp, list): failed_patterns.extend(fp)
            if isinstance(ra, list): recommended_actions.extend(ra)
            if notes: adaptation_notes.append({"save_id": save_id, "timestamp": str(ts), "note": notes})
            if isinstance(cs, dict):
                for key, val in cs.items():
                    capability_scores.setdefault(key, []).append(val)
            if include_recent_records and len(recent_records) < recent_limit:
                recent_records.append({"save_id": save_id, "timestamp": str(ts), "session_ref": session_ref, "drift_score": drift, "human_title": title, "human_summary": summary, "learning_events": le or [], "successful_patterns": sp or [], "failed_patterns": fp or [], "recommended_actions": ra or []})
        averaged_scores = {}
        for key, values in capability_scores.items():
            numeric = [v for v in values if isinstance(v, (int, float))]
            if numeric:
                averaged_scores[key] = round(sum(numeric) / len(numeric), 4)
        return ok({"user_id": user, "records_scanned": len(rows), "learning_events": learning_events[:200], "successful_patterns": successful_patterns[:200], "failed_patterns": failed_patterns[:200], "capability_scores": averaged_scores, "adaptation_notes": adaptation_notes[:100], "recommended_actions": recommended_actions[:200], "recent_records": recent_records})
    except Exception as e:
        return fail(f"Database error: {e}", 500)
